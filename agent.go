...
// coding agents (Claude Code, Codex, OpenCode). It abstracts away the
// different communication protocols used by each CLI.
package agent
...
type ExecOptions struct {
	...
	MaxTurns        int           // maximum number of turns (0 = unlimited)
	Timeout         time.Duration // execution timeout (0 = 20 minutes)
	ResumeSessionID string        // resume a previous session (if supported)
}
...
type Result struct {
	...
	SessionID  string // session ID for resumption (if supported)
}
